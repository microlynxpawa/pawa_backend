...
export class BusinessClientsService {
  ...
  public async AddNewBusinessClient(
    dto: AddNewBusinesslientDto
    ...
    try {
      ...
      await this.db.instance?.query(
        "CALL prcAddEditBusinessClient(:bankId, :clientId, :clientName, :registeredAddress, :officeAddress, :officePhone1, :officePhone2, :emailId1, :emailId2, :registrationDate, :registrationNumber, :tinNumber, :operatorId)",
        {
          replacements: {
            bankId,
            clientId,
      ...
      return BusinessClientsErrorCodes.success;
    } catch (error) {
      Logger.error("Error occurred while adding new bunisess client:", error);
      return BusinessClientsErrorCodes.newBusinessClientFailed;
  ...
  public async getAllBusinessClient(): Promise<any> {
    try {
      ...
      //throw error; // Re-throw the error for the caller to handle
      return BusinessClientsErrorCodes.fetchBusinessClientFailed;
    }
  }
}
